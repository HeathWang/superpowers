...
# Phase selection. Default to the current expected behavior; the historical RED
# baseline remains available via `red` when intentionally validating the old
# failure mode.
PHASE="${1:-green}"
...
if [ "$PHASE" = "red" ]; then
    echo "--- RED PHASE: Historical baseline WITHOUT Step 1a ---"
