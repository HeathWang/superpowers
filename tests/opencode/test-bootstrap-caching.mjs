...
const transformKind = plugin['experimental.chat.system.transform'] ? 'system' : 'messages';
...
function countBootstrapParts(output) {
  if (Array.isArray(output.system) && output.system.length > 0) {
    return output.system.filter((part) => String(part).includes('EXTREMELY_IMPORTANT')).length;
  }
...
function bootstrapText(output) {
  if (Array.isArray(output.system) && output.system.length > 0) {
    return output.system.find((part) => String(part).includes('EXTREMELY_IMPORTANT')) || '';
  }
