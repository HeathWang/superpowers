...
async function loadExtension() {
  ...
  const compiledPath = await compilePiExtensionForNodeTest();
  try {
    const mod = await import(pathToFileURL(compiledPath).href + `?cachebust=${Date.now()}-${Math.random()}`);
    mod.default(pi);
    return { handlers };
  } finally {
...
async function compilePiExtensionForNodeTest() {
  const compiledPath = resolve(dirname(extensionPath), `.superpowers-test-${process.pid}-${Date.now()}.mjs`);
  const source = await readFile(extensionPath, 'utf8');
  const compiled = source
    .replace(/^import type .*;\n/m, '')
    .replace('let cachedBootstrap: string | null | undefined;', 'let cachedBootstrap;')
    .replace('export default function superpowersPiExtension(pi: ExtensionAPI)', 'export default function superpowersPiExtension(pi)')
    .replace('function getBootstrapContent(): string | null', 'function getBootstrapContent()')
    .replace('function stripFrontmatter(content: string): string', 'function stripFrontmatter(content)')
    .replace('function piToolMapping(): string', 'function piToolMapping()')
    .replace('function messageContainsBootstrap(message: unknown): boolean', 'function messageContainsBootstrap(message)')
    .replace('function firstNonCompactionSummaryIndex(messages: unknown[]): number', 'function firstNonCompactionSummaryIndex(messages)')
    .replaceAll(' as const', '')
    .replace('(message as { content?: unknown }).content', 'message.content')
    .replace('(part as { type?: unknown }).type', 'part.type')
    .replace('(part as { text?: unknown }).text', 'part.text')
    .replace('(part as { text: string }).text', 'part.text')
    .replace('(messages[index] as { role?: unknown } | undefined)?.role', 'messages[index]?.role');
