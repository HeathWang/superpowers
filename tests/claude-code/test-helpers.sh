...
run_claude() {
    ...
    # Disable session persistence so prompt-based assertions do not inherit
    # state from a previous local Claude session.
    local cmd=(claude -p --no-session-persistence "$prompt")
